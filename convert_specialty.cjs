...
// RELAXED REGEX for Specialty Pages: matches any array of strings
const whyChooseMatch = content.match(/\{\[\s*[\s\S]*?\]\.map/); // Very generic, might need refinement if overlaps
// Let's refine:
// {["String", "String"].map((reason, idx) => ( ... ))}
// Try: /\{\[\s*"[^"]+"[\s\S]*?\]\.map/
// Actually, let's look at source: 
// {[ "Director...", "Pioneered..." ].map((reason, idx) => (
const genericWhyChooseMatch = content.match(/\{\[\s*"[^"]+"[\s\S]*?\]\.map/);

if (genericWhyChooseMatch) {
    const listStr = genericWhyChooseMatch[0];
...
// Why Choose Loop (Generalized)
const whyChooseLoopRegex = /\{\[\s*"[^"]+"[\s\S]*?\]\.map\(\(reason, idx\) => \(\s*<li[\s\S]*?<\/li>\s*\)\)\}/;
